...
DataDeserializer::DataDeserializer(const uint8_t *buffer, uint32_t offset) : buffer(buffer), index(offset) {}

uint32_t DataDeserializer::parseUInt32() {
    uint32_t result = buffer[index + INT_SIZE - 1];
    for (size_t i = index + INT_SIZE - 1; i > index; --i) {
        result <<= 8;
        result += buffer[i - 1];
    }
    index += INT_SIZE;
...
std::vector<model::EmailInfo> DataDeserializer::parseEmailInfos() {
    ...
    std::vector<model::EmailInfo> result;
    result.reserve(size);
