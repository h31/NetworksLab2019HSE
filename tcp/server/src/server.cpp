...
void
RouletteServer::WorkWithClient(RouletteServer* server, int sock_fd, const sockaddr_in& cli_addr) {
...
void RouletteServer::StartServer(uint16_t port_number) {
    ...
    while (true) {
        ...
        std::thread thread(WorkWithClient, this, new_socket_fd, cli_addr);
    }
}
